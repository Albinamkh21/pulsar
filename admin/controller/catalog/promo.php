...
class ControllerCatalogPromo extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_promo->editPromo($this->request->get['promo_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $promo_id) {
				$this->model_catalog_promo->deletePromo($promo_id);
			}
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['promos'][] = array(
				...
				'edit'           => $this->url->link('catalog/promo/edit', 'token=' . $this->session->data['token'] . '&promo_id=' . $result['promo_id'] . $url, true)
			);
		}
		...
		$data['sort_title'] = $this->url->link('catalog/promo', 'token=' . $this->session->data['token'] . '&sort=id.title' . $url, true);
		$data['sort_sort_order'] = $this->url->link('catalog/promo', 'token=' . $this->session->data['token'] . '&sort=i.sort_order' . $url, true);
	...
	protected function getForm()
    {
        ...
        if (isset($this->request->post['promo_description'][1])) {
            $desc = $this->request->post['promo_description'][1];
            $this->request->post['image'] = $desc['image'];
        }

        $this->load->model('tool/image');
        if (isset($desc['image']) && is_file(DIR_IMAGE . $desc['image'])) {
            $data['thumb'] =$this->model_tool_image->resize($desc['image'], 100, 100);
		...
		if (!isset($this->request->get['promo_id'])) {
			...
			$data['action'] = $this->url->link('catalog/promo/edit', 'token=' . $this->session->data['token'] . '&promo_id=' . $this->request->get['promo_id'] . $url, true);
		}
		...
		if (isset($this->request->get['promo_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$promo_info = $this->model_catalog_promo->getPromo($this->request->get['promo_id']);
		}
		...
		if (isset($this->request->post['promo_description'])) {
			...
			$data['promo_description'] = $this->model_catalog_promo->getPromoDescriptions($this->request->get['promo_id']);
           // $data['promo_description'][1]['image'] =
            $data['thumb'] = $this->model_tool_image->resize($data['promo_description'][1]['image'], 100, 100);
		...
		if (isset($this->request->post['promo_store'])) {
			$data['promo_store'] = $this->request->post['promo_store'];
		} elseif (isset($this->request->get['promo_id'])) {
			$data['promo_store'] = $this->model_catalog_promo->getPromoStores($this->request->get['promo_id']);
		} else {
		...
		if (isset($this->request->post['keyword'])) {
			...
			$data['keyword'] = $promo_info['keyword'];
		} else {
			$data['keyword'] = '';
		}
		...
		if (isset($this->request->post['sort_order'])) {
			...
			$data['sort_order'] = $promo_info['sort_order'];
		} else {
			$data['sort_order'] = '';
		}

		if (isset($this->request->post['promo_layout'])) {
			$data['promo_layout'] = $this->request->post['promo_layout'];
		} elseif (isset($this->request->get['promo_id'])) {
			$data['promo_layout'] = $this->model_catalog_promo->getPromoLayouts($this->request->get['promo_id']);
		} else {
			$data['promo_layout'] = array();
		}
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['promo_description'] as $language_id => $value) {
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			...
			if ($url_alias_info && isset($this->request->get['promo_id']) && $url_alias_info['query'] != 'promo_id=' . $this->request->get['promo_id']) {
				$this->error['keyword'] = sprintf($this->language->get('error_keyword'));
			}

			if ($url_alias_info && !isset($this->request->get['promo_id'])) {
				$this->error['keyword'] = sprintf($this->language->get('error_keyword'));
			}
		}
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $promo_id) {
			if ($this->config->get('config_account_id') == $promo_id) {
				$this->error['warning'] = $this->language->get('error_account');
			}

			if ($this->config->get('config_checkout_id') == $promo_id) {
				$this->error['warning'] = $this->language->get('error_checkout');
			}

			if ($this->config->get('config_affiliate_id') == $promo_id) {
				$this->error['warning'] = $this->language->get('error_affiliate');
			}

			if ($this->config->get('config_return_id') == $promo_id) {
				$this->error['warning'] = $this->language->get('error_return');
			}

			/*$store_total = $this->model_setting_store->getTotalStoresByPromoId($promo_id);
