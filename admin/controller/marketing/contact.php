...
class ControllerMarketingContact extends Controller {
	...
	public function send() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!$json) {
                ...
                $this->load->model('catalog/subscriber');
				...
				switch ($this->request->post['to']) {
                    ...
                    case 'subscibers':
                            ...
                            $email_total = $this->model_catalog_subscriber->getTotalEmailsSubscribers();
                            $results = $this->model_catalog_subscriber->getEmailsSubscribers(($page - 1) * 10, 10);
				...
				if ($emails) {
					...
					$json['success'] = sprintf($this->language->get('text_sent'), count($emails), count($email_total));
					...
					foreach ($emails as $email) {
						if (preg_match($this->config->get('config_mail_regexp'), $email)) {
                            $this->sendMail($email,$this->request->post['subject'],$message);
    ...
    public function sendMail($email, $subject, $body){
        ...
        $mail->addAddress($email, ''); // адрес почты, на который будет доставлено письмо
        /*foreach($emails as $email)
        {
