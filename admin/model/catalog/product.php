...
class ModelCatalogProduct extends Model {
    ...
    public function UploadDataToTmpTable($file) {

        $query = $this->db->query("truncate table tmp_products");
        ...
        $sql = "update `tmp_products` set category_id  = coalesce((select category_id from  oc_category_description cd where cd.name=tmp_products.category),0)";
        $query = $this->db->query($sql);

        $sql = "update `tmp_products` set weight_class_id = coalesce((select weight_class_id  from  oc_weight_class_description  cd where cd.unit=tmp_products.units),0)";
        $query = $this->db->query($sql);
    ...
    public function UploadDataToTable() {
        $sql = "delete from oc_product where product_id in (select id from `tmp_products` );";
        $query = $this->db->query($sql);

        $sql = "insert into oc_product(product_id, model, sku, upc, ean, price, quantity, location, weight_class_id,
        ...
        $sql = "delete from `oc_product_description` where product_id in (select id from `tmp_products` );";
        $query = $this->db->query($sql);

        $sql = "insert into `oc_product_description`(product_id, name, description, language_id, tag,meta_title,meta_h1,meta_description,meta_keyword)
                select id, name, info, 1,'','','','',''  FROM `tmp_products`";
        $query = $this->db->query($sql);


        $sql = "delete from `oc_product_to_category` where product_id in (select id from `tmp_products` );";
        $query = $this->db->query($sql);
        ...
        $sql = "delete from `oc_product_to_store` where product_id in (select id from `tmp_products` );";
        $query = $this->db->query($sql);
