    ...
    public function addOrderSimple($data) {
        ...
        */
        $this->db->query("INSERT INTO `" . DB_PREFIX . "order` 
		                    ...
		                    payment_firstname = '" . $this->db->escape($data['name']) . "', payment_lastname = '', 
		                    payment_company = '', 
		                    payment_address_1 = '" . $this->db->escape($data['address_1']) . "', 
		                    payment_address_2 = '', payment_city = '" . $this->db->escape($data['city']) . "', 
		                    payment_postcode = '',
		                    payment_country = 'kz', 
		                    payment_country_id = '0', payment_zone = '', 
		                    ...
		                    payment_code = '" . $this->db->escape($data['payment_code']) . "', shipping_firstname = '" . $this->db->escape($data['name']) . "', 
		                    shipping_lastname = '', 
		                    ...
		                    comment = '" . $this->db->escape($data['comments']) . "', 
		                    total = '" . (float)$data['total'] . "', 
		                    order_status_id = 1,
		                    affiliate_id = '" . (int)$data['affiliate_id'] . "', 
		                    commission = '" . (float)$data['commission'] . "', marketing_id = '', tracking = '', 
		                    language_id = '" . (int)$this->config->get('config_language_id') . "',
		                    currency_id = '" . (int)$data['currency_id'] . "', currency_code = '" . $this->db->escape($data['currency_code']) . "',
		                    currency_value = '" . (float)$data['currency_value'] . "', 
		                    ip = '" . $this->db->escape($data['ip']) . "', forwarded_ip = '" .  $this->db->escape($data['forwarded_ip']) . "', 
		                    user_agent = '" . $this->db->escape($data['user_agent']) . "', 
		                    accept_language = '" . $this->db->escape($data['accept_language']) . "', date_added = NOW(), date_modified = NOW()");
        ...
        if (isset($data['products'])) {
            foreach ($data['products'] as $product) {
                ...
                SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', 
                model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', 
                price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "',
                tax = 0, reward = '" . (int)$product['reward'] . "'");
