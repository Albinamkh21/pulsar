...
$(document).ready(function() {
    ...
    $('#subscribe_link').on('click', function(e) {
        ...
        $.ajax({
            url: 'index.php?route=common/subscribe/save',
            type: 'post',
            data: 'email=' + email,
            dataType: 'json',
            beforeSend: function() {
	...
	$('#search input[name=\'search\']').on('click', function() {
		...
		var value = $('header input[name=\'search_value\']').val();
        console.log(value);
		...
		$.ajax({
			...
			success: function(json) {
				...
				if (json['success']) {
					...
					setTimeout(function () {
						$('#basket-count').html(json['total_cnt']);
						if(json['total_cnt']>0) { $('.make-order-label__link').html('<a href="index.php?route=checkout/cart"> <span class="make-order-label make-order-label_active">Оформить заказ</span> </a>'); }
						else { $('.make-order-label__link').html(' <a href="index.php?route=common/catalog" ><span class="make-order-label">Сформируйте новый заказ</span> </a>'); }
