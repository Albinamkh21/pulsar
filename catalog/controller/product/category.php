...
class ControllerProductCategory extends Controller {
	public function index() {
        ...
        {
            $filter_names_ids = "'" . implode ( "', '", $this->request->post['filter']) . "'";
            $data['filter_clear'] = '1';
        ...
        if(isset($this->request->post['gosts']) && $this->request->post['gosts'])
        {
            $filter_gosts_ids = "'" . implode ( "', '", $this->request->post['gosts'] ) . "'";
            $data['filter_clear'] = '2';
        ...
        $data['instore'] = "";
        $filter_instore = "";
        if(isset($this->request->get['instore']) && $this->request->get['instore']){

            $filter_instore = $this->request->get['instore'];
            $data['instore'] = "checked";
        }
		...
		if ($category_info) {
			...
			$filter_data = array(
				...
				'start'              => ($page - 1) * $limit,
				'limit'              => $limit,
                'filter_products'    => $filter_names_ids,
                'filter_gosts'       => $filter_gosts_ids,
                'filter_instore'     => $filter_instore
...
*/
            ...
            $data['sorts'][] = array(
                'text'  => $this->language->get('text_year_desc'),
                'value' => 'p.upc-DESC',
                'href'  => $this->url->link('product/category', 'path=' . $path . '&sort=p.upc&order=DESC' . $url)
            );
            $data['sorts'][] = array(
                'text'  => $this->language->get('text_year_asc'),
                'value' => 'p.upc-ASC',
                'href'  => $this->url->link('product/category', 'path=' . $path . '&sort=p.upc&order=ASC' . $url)
            );
