...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
        ...
        (SELECT wd.unit FROM  " . DB_PREFIX . "weight_class_description wd   WHERE wd.weight_class_id = p.weight_class_id ) as units  ";
        ...
        if (!empty($data['filter_products'])) {
            $sql .= " AND pd.name in  (" .$data['filter_products'] . ")";
        }
        if (!empty($data['filter_gosts'])) {
            $sql .= " AND p.ean in  (" . $data['filter_gosts'] . ")";
        }
        if (!empty($data['filter_instore'])) {
            if($data['filter_instore']==1)
